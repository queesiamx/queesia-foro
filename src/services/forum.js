...
const DB = db;
// ---------- Helpers ----------
export function computeTrendingScore(t) {
...
// ---------- Live queries ----------
export function watchTrendingThreads(onData, opts = {}) {
    const pageSize = opts.pageSize ?? 10;
    // Intenta con trendingScore
    const q1 = query(collection(DB, "threads"), where("status", "in", ["published", "open"]), // <—
    orderBy("trendingScore", "desc"), limit(pageSize));
    // Fallback por fecha (y reordenamos en cliente)
    const q2 = query(collection(DB, "threads"), orderBy("lastActivityAt", "desc"), limit(pageSize * 2));
    const handleQ2 = (snap2) => {
        let items = snap2.docs.map((d) => ({
            id: d.id,
            ...d.data(),
        }));
        items = items
    ...
    const unsubQ1 = onSnapshot(q1, (snap) => {
        ...
        if (err.code === "failed-precondition" || /index/i.test(String(err?.message))) {
            unsubQ2 = onSnapshot(q2, handleQ2);
        }
        else {
            console.error(err);
...
export async function getSidebarCounts() {
    ...
    const totalQ = query(collection(DB, "threads"), where("status", "in", baseStatus));
    const recientesQ = query(collection(DB, "threads"), where("status", "==", "published"));
    const preguntasQ = query(collection(DB, "threads"), where("status", "==", "published"), where("tags", "array-contains", "preguntas"));
    const tutorialesQ = query(collection(DB, "threads"), where("status", "==", "published"), where("tags", "array-contains", "tutorial"));
